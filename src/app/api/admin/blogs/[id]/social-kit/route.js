...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const posts = Object.fromEntries(["linkedin", "facebook", "x", "whatsapp"].map((key) => [key, String(body[key] || "").trim()]));
    if (Object.values(posts).some((value) => !value)) {
      return NextResponse.json({ success: false, error: "All four social posts are required." }, { status: 400 });
    }
