...
export async function GET(request) {
  try {
    ...
    console.log(`[GET] Connected to DB: ${dbName}, State: ${mongoose.connection.readyState}`);
...
export async function PATCH(request) {
  try {
    await dbConnect();
    const dbName = mongoose.connection.db.databaseName;
    console.log(`[PATCH] Connected to DB: ${dbName}, State: ${mongoose.connection.readyState}`);
    ...
    const savedConfig = await SiteConfig.findOneAndUpdate(
    ...
    return apiResponse.success(savedConfig.toObject(), "Settings updated successfully");
  } catch (error) {
