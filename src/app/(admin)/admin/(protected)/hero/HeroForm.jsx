...
export default function HeroForm({ initialData, staticFallback }) {
    ...
    const defaultValues = {
        ...
        visualImageAlt: initialData?.visualImageAlt || "",
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const formattedData = {
                ...
                visualImageAlt: data.visualImageAlt,
    ...
    return (
        <div className="max-w-[1300px] mx-auto pb-40 px-4 md:px-0">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 mb-16 px-6">
                <div>
                   <h1 className="text-5xl font-black italic uppercase tracking-tighter text-foreground">Hero <span className="text-accent underline decoration-accent/20 underline-offset-8">Management</span></h1>
                ...
                <Button
                    ...
                    disabled={isSaving}
                    className="h-20 w-full lg:w-72 rounded-3xl font-black uppercase tracking-[0.2em] shadow-xl shadow-accent/20 hover:shadow-2xl hover:shadow-accent/30 transition-all bg-accent text-accent-foreground"
                >
            ...
            <div className="space-y-12">
                ...
                <motion.div
                    ...
                    className="glass p-10 md:p-14 rounded-[3.5rem] border border-border/70 relative overflow-hidden group"
                >
                    ...
                    <div className="relative w-full min-h-[400px] rounded-[2.5rem] bg-overlay/40 border-2 border-dashed border-border hover:border-accent/40 transition-all p-8">
                        <Controller
                            name="visualImage"
                            control={control}
                            render={({ field }) => (
                    ...
                    <input
                        {...register("visualImageAlt")}
                        className="mt-6 w-full bg-overlay/40 border border-border rounded-2xl px-6 py-4 outline-none"
                        placeholder="Accessible hero image description"
                    />
                </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <motion.div
                         ...
                         className="glass p-10 md:p-14 rounded-[3.5rem] border border-border/70 space-y-8"
                    >
                        ...
                        <textarea
                            ...
                            className="w-full bg-overlay/40 border border-border rounded-[2rem] p-8 min-h-[300px] focus:ring-4 focus:ring-accent/10 outline-none transition-all font-medium leading-relaxed text-lg"
                            ...
                            {typewriterFields.map((field, index) => (
                                <div key={field.id} className="flex gap-4 items-center">
                                    <input
                                        ...
                                        className="flex-1 bg-overlay/20 border border-border/70 rounded-2xl px-6 py-4 outline-none font-bold"
                                    />
                ...
                <motion.div
                     initial={{ opacity: 0, y: 30 }}
                     animate={{ opacity: 1, y: 0 }}
                     className="glass p-10 md:p-14 rounded-[3.5rem] border border-border/70"
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {featureFields.map((field, index) => (
                            <div key={field.id} className="p-8 bg-card/50 border border-border/70 rounded-[2.5rem] relative group hover:bg-muted/50 transition-all">
                                <button type="button" onClick={() => removeFeature(index)} className="absolute top-6 right-6 text-muted-foreground/20 hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
                                <div className="space-y-5 mt-4">
                                     <div className="grid gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black uppercase text-accent/40 tracking-widest italic ml-1">Icon</label>
                                            <select {...register(`features.${index}.icon`)} className="w-full bg-overlay/60 border border-border rounded-xl px-4 py-3 text-xs outline-none">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <input {...register(`features.${index}.label`)} className="w-full bg-overlay/60 border border-border rounded-xl px-4 py-3 text-xs outline-none" />
                                        </div>
                                     </div>
                                     <div className="space-y-1">
                                        <label className="text-[10px] font-black uppercase text-accent/40 tracking-widest italic ml-1">Description</label>
                                        <input {...register(`features.${index}.description`)} className="w-full bg-overlay/60 border border-border rounded-xl px-4 py-3 text-xs outline-none" />
                                     </div>
                                </div>
                            </div>
                        ))}
