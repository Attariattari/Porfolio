...
export default function AdminSearchPage() {
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      case "Blog": return <FileText className="w-5 h-5 text-accent" />;
      ...
      default: return <SettingsIcon className="w-5 h-5 text-muted-foreground" />;
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground mt-4 font-medium tracking-widest uppercase">
          ...
          Object.entries(groupedResults).map(([type, items]) => (
            <section key={type} className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-xs font-black uppercase tracking-[0.4em] text-muted-foreground/80 flex items-center gap-3">
                  {getTypeIcon(type)}
                  {type}s
                  <span className="px-2 py-0.5 bg-muted/50 rounded text-[10px] text-muted-foreground">
                    {items.length}
                  </span>
                </h2>
                <div className="h-px flex-1 bg-muted/50" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {items.map((item) => (
                  <Link
                    key={item.id}
                    href={item.route}
                    className="group p-6 bg-card/50 border border-border/70 rounded-[2rem] hover:bg-muted/50 hover:border-border transition-all duration-500 flex items-center justify-between shadow-xl"
                  >
                    <div className="space-y-2 flex-1 min-w-0">
                      <h3 className="text-lg font-bold text-foreground group-hover:text-accent transition-colors truncate">
                        {item.title}
                      </h3>
                      <p className="text-xs text-muted-foreground truncate-2-lines leading-relaxed pr-4 italic">
                        {item.description || "No description available for this record."}
                      </p>
                    </div>
                    <div className="w-12 h-12 rounded-2xl bg-muted/50 border border-border/70 flex items-center justify-center group-hover:bg-accent group-hover:border-accent transition-all duration-500 group-hover:rotate-12 group-hover:scale-110 shadow-lg">
