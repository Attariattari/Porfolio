...
import { CheckCircle2, Copy, Download, ExternalLink, Facebook, FileText, ImageIcon, Linkedin, Loader2, RefreshCw, Save, Send, Share2, Sparkles, X } from "lucide-react";
...
const emptyPosts = { linkedin: "", facebook: "", x: "", whatsapp: "" };
...
export default function SocialShareKitModal({ blog, isOpen, onClose, onUpdated }) {
  ...
  const platformUrl = () => {
    ...
    return `https://wa.me/?text=${encodeURIComponent(posts.whatsapp)}`;
  };
  ...
  return createPortal(
    <div className="social-share-kit-theme fixed inset-0 z-[1200] flex h-[100dvh] w-screen items-end justify-center bg-background/90 p-0 backdrop-blur-xl md:items-center md:p-5" role="dialog" aria-modal="true" aria-labelledby="social-kit-title">
      ...
      <section className="relative flex max-h-[97vh] w-full max-w-[1280px] flex-col overflow-hidden rounded-t-[28px] border border-border bg-card shadow-2xl md:max-h-[94vh] md:rounded-[28px]">
        ...
        <header className="relative flex items-start justify-between gap-5 border-b border-white/[0.08] px-5 py-5 sm:px-7 sm:py-6">
          <div className="flex min-w-0 items-start gap-4">
            <span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-violet-400/10 text-violet-300 ring-1 ring-inset ring-violet-300/15"><Share2 className="size-5" /></span>
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-[9px] font-bold uppercase tracking-[.24em] text-violet-300">Social publishing studio</p><span className={`rounded-full border px-2 py-1 text-[8px] font-bold uppercase tracking-wider ${readyCount === 4 ? "border-emerald-300/20 bg-emerald-400/10 text-emerald-300" : "border-amber-300/20 bg-amber-400/10 text-amber-300"}`}>{readyCount}/4 ready</span></div><h2 id="social-kit-title" className="mt-2 text-xl font-semibold tracking-[-.025em] text-white sm:text-2xl">Campaign-ready social content</h2><p className="mt-1 truncate text-xs text-slate-500">{blog.title}</p></div>
          </div>
        ...
        <div className="grid min-h-0 flex-1 xl:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="min-h-0 overflow-y-auto border-b border-white/[0.08] bg-slate-950/20 p-5 xl:border-b-0 xl:border-r sm:p-6">
            <div className="mb-3 flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-slate-600">Campaign visual</p><p className="mt-1 text-xs font-semibold text-slate-300">Featured blog image</p></div><span className={`flex items-center gap-1.5 text-[9px] font-bold ${imageUrl ? "text-emerald-300" : "text-amber-300"}`}>{imageUrl ? <CheckCircle2 className="size-3" /> : <ImageIcon className="size-3" />}{imageUrl ? "Ready" : "Missing"}</span></div>
            <div className="group relative aspect-video overflow-hidden rounded-2xl border border-white/[0.1] bg-slate-950/70 shadow-xl">{imageUrl ? <Image src={getSafeImageSrc(imageUrl)} alt={blog.featuredImage?.alt || `${blog.title} social image`} fill sizes="360px" className="object-cover transition duration-700 group-hover:scale-[1.025]" /> : <div className="grid size-full place-items-center"><ImageIcon className="size-8 text-slate-700" /></div>}<div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-950/55 via-transparent to-transparent" /></div>
            {imageUrl && <a href={imageUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.025] text-xs font-bold text-slate-400 transition hover:border-white/15 hover:text-white"><Download className="size-4" />Open or download visual</a>}
          ...
          <div className="flex min-h-0 flex-col bg-[#0a1423]">
            ...
            <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_300px]">
              <div className="flex min-h-0 flex-col p-5 sm:p-6">
                ...
                <textarea value={posts[active]} onChange={(event) => setPosts((current) => ({ ...current, [active]: event.target.value }))} placeholder="Generate the Social Share Kit to prepare this platform post." className="flex-1 w-full resize-none rounded-2xl border border-white/[0.08] bg-slate-950/35 p-5 text-sm leading-7 text-slate-200 shadow-inner outline-none placeholder:text-slate-700 focus:border-violet-400/40 focus:ring-4 focus:ring-violet-400/[0.04]" />
              </div>
