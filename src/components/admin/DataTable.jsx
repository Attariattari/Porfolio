...
function SortableRow({ item, columns, onEdit, onDelete, onView, highlightedItem, onReorder }) {
  ...
  return (
    <tr
      ref={setNodeRef}
      style={style}
      className={`group border-b border-border/40 transition-all duration-300 ${
    ...
    >
      <td className="p-5 w-10">
        <button
          {...attributes}
          {...listeners}
          className="p-1 cursor-grab active:cursor-grabbing text-muted-foreground hover:text-accent transition-colors opacity-0 group-hover:opacity-100"
        >
        ...
        <td key={col.key} className="p-5">
          <div className={`transition-all ${highlightedItem === (item._id || item.id) ? "font-black text-accent scale-[1.02] origin-left" : ""}`}>
            {col.render ? (
              ...
              <span className={`text-sm tracking-tight ${highlightedItem === (item._id || item.id) ? "font-black text-accent" : "font-semibold text-foreground"}`}>
                {item[col.key]}
              </span>
            )}
          </div>
      ...
      <td className="p-5">
        <div className="flex items-center justify-end gap-2">
          <button
            ...
            className="p-2.5 rounded-xl hover:bg-muted text-muted-foreground hover:text-accent transition-all"
          >
    ...
    >
      <div className="absolute left-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
           {...attributes}
           {...listeners}
           className="p-2 cursor-grab active:cursor-grabbing text-muted-foreground hover:text-accent"
        >
          <GripVertical className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="pl-6 space-y-4">
        {columns.map((col) => (
          <div key={col.key} className="flex flex-col gap-1">
            ...
            <div className={`text-sm tracking-tight transition-all ${highlightedItem === (item._id || item.id) ? "font-black text-accent" : "font-semibold text-foreground"}`}>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-border/50">
        <button
          onClick={() => onView && onView(item)}
          className="p-3 rounded-xl bg-card border border-border text-muted-foreground hover:text-accent transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"
        >
        ...
        <button
          onClick={() => onDelete(item)}
          className="p-3 rounded-xl bg-destructive/10 border border-destructive/10 text-destructive/70 hover:text-destructive transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"
        >
...
export default function DataTable({
  ...
  const renderMobileContent = () => {
    if (paginatedData.length === 0) {
      return (
        <div className="py-20 text-center text-muted-foreground font-medium italic opacity-60 uppercase text-[10px] tracking-widest">
      ...
      return (
        <tr>
          <td
            ...
            className="py-20 text-center text-muted-foreground font-medium italic opacity-60 uppercase text-[10px] tracking-widest"
          >
            No data available in this module.
          </td>
        </tr>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border/50 bg-muted/20">
              ...
              {columns.map((col) => (
                <th key={col.key} className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-80">
                  {col.label}
                </th>
              ))}
              <th className="p-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-80 text-right">
                Actions
              </th>
            </tr>
          </thead>
  ...
  return (
    <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-2xl backdrop-blur-sm">
      <div className="p-8 border-b border-border/50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-muted/20">
          ...
          {filters.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {filters.map((filter) => (
                <select
                  key={filter.key}
                  className="bg-background/60 border border-border rounded-lg px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground focus:outline-none focus:border-accent/40"
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          <div className="relative w-full lg:w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-background/60 border border-input rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/20 transition-all font-medium"
          ...
          {onAdd && (
            <button
              onClick={onAdd}
              className="flex items-center gap-2 px-6 py-3 bg-accent text-accent-foreground font-black uppercase text-xs tracking-widest rounded-xl hover:bg-accent/90 transition-all shadow-lg shadow-accent/20 active:scale-95 whitespace-nowrap"
            >
      ...
      <div className="p-6 border-t border-border/50 flex items-center justify-between bg-muted/20">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2.5 rounded-xl border border-border hover:bg-muted text-muted-foreground transition-all disabled:opacity-20 disabled:cursor-not-allowed"
          >
