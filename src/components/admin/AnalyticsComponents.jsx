...
import { Globe, Monitor, MapPin, FileText, Users, TrendingUp, ExternalLink } from 'lucide-react';
                        ...
                        {key.toLowerCase().includes('rate') ? (
                          <div className="flex items-center gap-2">
                            <div className={`w-1.5 h-1.5 rounded-full ${
                              value < 30 ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 
                              value > 70 ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 
        ...
        devices?.map((device, i) => {
          ...
          return (
            <motion.div
              ...
              className="p-6 border border-white/5 bg-white/[0.02] rounded-2xl hover:border-white/10 transition-all group"
            >
 ...
 * Location Card
 */
export const LocationCard = ({ location, type = 'country' }) => {
  return (
    <motion.div
      initial={{ opacity: 0, x: -10 }}
      animate={{ opacity: 1, x: 0 }}
      className="p-4 border border-white/5 bg-white/[0.02] rounded-xl hover:border-white/10 transition-all group"
    >
      ...
      <div className="w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
        <motion.div 
          initial={{ width: 0 }}
          animate={{ width: `${(location.visitors / 100) * 100}%` }}
          className="h-full bg-gradient-to-r from-accent to-blue-500 shadow-[0_0_10px_rgba(var(--accent-rgb),0.3)]"
        />
      </div>
    </motion.div>
  );
...
export const RealTimeActivityFeed = ({ activities, isLoading }) => {
  return (
    <div className="border border-white/5 bg-white/[0.01] rounded-[2rem] p-8 relative overflow-hidden group">
        ...
        {isLoading ? (
          ...
          activities.map((activity, i) => (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: i * 0.05 }}
              className="p-4 border border-white/5 bg-white/[0.02] rounded-xl hover:border-white/10 transition-all border-l-2 border-l-transparent hover:border-l-accent"
            >
 ...
 * Metric Card
 */
export const MetricCard = ({ label, value, trend, icon: Icon, color = 'blue' }) => {
  const colorClasses = {
  ...
  return (
    <motion.div
      ...
      className="p-8 border border-white/5 bg-white/[0.02] rounded-3xl relative overflow-hidden group transition-all duration-500"
    >
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        {trend && (
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 mb-1">Growth</span>
            <span className="text-xs font-black text-emerald-400 flex items-center gap-1 bg-emerald-500/10 px-2 py-1 rounded-full border border-emerald-500/20">
