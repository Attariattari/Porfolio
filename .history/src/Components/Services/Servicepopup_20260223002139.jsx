    ...
    <div className={`Serviceall ${scrollDisabled ? "scroll-disabled" : ""}`}>
      {info && (
        <div className="Servicepopup" onClick={closePopup}>
          <div className="Servicepopupchild" onClick={(e) => e.stopPropagation()}>
            <button className="Closeone" onClick={closePopup} aria-label="Close">
              <CgCloseR size={24} />
            </button>
            ...
            <div className="popup-header">
              ...
              <h2 className="popup-title font-serif">
                {info.title}
              </h2>
            </div>
          ...
          <div className="Newspopupchild">
            <div className="news-main-content">
              ...
              <div className="news-intro-section">
                <h1 className="intro-text-large">{selectedNews.NewsSection} Overview</h1>
              ...
              <div className="chapters-flow">
                {[
                  ...
                  chapterSet.data && chapterSet.data.length > 0 && (
                    <div key={chapterSet.id} className="chapter-container">
                      <span className="chapter-badge">Chapter {chapterSet.id}</span>
                      {chapterSet.data.map((chapter, index) => (
                        <div key={index} className="mb-8">
                          <h2 className="chapter-title">{chapter.title}</h2>
                          <div className="chapter-details">{chapter.details}</div>
                        </div>
                      ))}
                    </div>
                  )
